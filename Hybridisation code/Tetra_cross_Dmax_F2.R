...
getcrosses <- function(xp1, xp2, nsamps=nsamps,params,dmax=NULL,opt=c(0,0))
{
  ...
  # (1) Get the fitness for the parental population, and the F1 
	...
	SP1 <- sum((zp1-opt)^2)
 	...
 	spgm3  <- split(gamete_matrix3,cg)
 	spgm4  <- split(gamete_matrix4,cg)

 	spgmP1 <- split(gamete_matrixP1,col(gamete_matrixP1))
 	spgmP2 <- split(gamete_matrixP2,col(gamete_matrixP2))
 	SF2  <- mapply(FUN=getS,spgm1,spgm2,spgm3,spgm4, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
...
xp1 = read.csv("~/Desktop/Simulation_output/Tetraploid/Parameter6/p6r5.txt",sep="\t")#substitution file for P1
xp2 = read.csv("~/Desktop/Simulation_output/Tetraploid/Parameter6/p6r8.txt",sep="\t")#substitution file for P1#substitution file for P2
...
plot_data_Tetra_F2 <- data.frame(cbind(x=c(1:199),plot_data))

require(ggplot2)
ggplot(plot_data, aes(x = plot_data$x, y = plot_data$wF2m)) +
  geom_point(size = 0.5) +
  geom_errorbar(aes(ymax = plot_data$wF2ub, ymin = plot_data$wF2lb)) + 
  labs(y= "fitness",x="Dmax")


save(plot_data_Tetra_F2, file="plot_data_Tetra_F2.RData")