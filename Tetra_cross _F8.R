...
getS <- function(g1,g2,g3,g4,zm,doms,zstart,opt=c(1,0)) 
{ 
	
	m <- rbind(zm*g1*0.25,zm*g2*0.25,zm*g3*0.25,zm*g4*0.25)
	if(!is.null(dim(m)))
	{	
...
getcrosses <- function(xp1, xp2, nsamps=nsamps,params,dmax=NULL,opt=c(0,0))
{
  ...
  # (1) Get the fitness for the parental population, and the F1 
	...
	SP1 <- sum((zp1-opt)^2)
 	...
 	gamete_matrix3  <- matrix(rbinom(nsamps*d,1,0.5),d,nsamps)
 	gamete_matrix4  <- matrix(rbinom(nsamps*d,1,0.5),d,nsamps)
 	...
 	spgm3F1  <- split(gamete_matrix3,cg)
 	spgm4F1  <- split(gamete_matrix4,cg)
 	spgmP1 <- split(gamete_matrixP1,col(gamete_matrixP1))
 	spgmP2 <- split(gamete_matrixP2,col(gamete_matrixP2))
 	
 	F2ind <- mapply(FUN=rbind,spgm1F1,spgm2F1,spgm3F1,spgm4F1,SIMPLIFY =F)
 	...
 	spgm3F2 <- lapply(F2ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F2 <- lapply(F2ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	
 	F3ind <- mapply(FUN=rbind,spgm1F2,spgm2F2,spgm3F2,spgm4F2,SIMPLIFY =F)
 	...
 	spgm3F3 <- lapply(F3ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F3 <- lapply(F3ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	F4ind <- mapply(FUN=rbind,spgm1F3,spgm2F3,spgm3F3,spgm4F3,SIMPLIFY =F)
 	...
 	spgm3F4 <- lapply(F4ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F4 <- lapply(F4ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	F5ind <- mapply(FUN=rbind,spgm1F4,spgm2F4,spgm3F4,spgm4F4,SIMPLIFY =F)
 	...
 	spgm3F5 <- lapply(F5ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F5 <- lapply(F5ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	F6ind <- mapply(FUN=rbind,spgm1F5,spgm2F5,spgm3F5,spgm4F5,SIMPLIFY =F)
 	...
 	spgm3F6 <- lapply(F6ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F6 <- lapply(F6ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	F7ind <- mapply(FUN=rbind,spgm1F6,spgm2F6,spgm3F6,spgm4F6,SIMPLIFY =F)
 	...
 	spgm3F7 <- lapply(F7ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	spgm4F7 <- lapply(F7ind,FUN=function(x) {apply(x,2,FUN=sample,size=1)})
 	
 	SF2  <- mapply(FUN=getS,spgm1F1,spgm2F1,spgm3F1,spgm4F1, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF3  <- mapply(FUN=getS,spgm1F2,spgm2F2,spgm3F2,spgm4F2, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF4  <- mapply(FUN=getS,spgm1F3,spgm2F3,spgm3F3,spgm4F3, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF5  <- mapply(FUN=getS,spgm1F4,spgm2F4,spgm3F4,spgm4F4, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF6  <- mapply(FUN=getS,spgm1F5,spgm2F5,spgm3F5,spgm4F5, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF7  <- mapply(FUN=getS,spgm1F6,spgm2F6,spgm3F6,spgm4F6, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	SF8  <- mapply(FUN=getS,spgm1F7,spgm2F7,spgm3F7,spgm4F7, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	# SB1  <- mapply(FUN=getS,spgm1F1,spgm2F1,spgmP1,spgmP1, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
 	# SB2  <- mapply(FUN=getS,spgm1F1,spgm2F1,spgmP2,spgmP2, MoreArgs=list(zm=zm,doms=doms,zstart=zp1))
...
xp1 = read.csv("~/Desktop/Simulation_output/Tetraploid/Parameter2/p2r7.txt",sep="\t")#substitution file for P1
xp2 = read.csv("~/Desktop/Simulation_output/Tetraploid/Parameter2/p2r8.txt",sep="\t")#substitution file for P1#substitution file for P2
  

res <- getcrosses(xp1,xp2,nsamps=nsamps,params,dmax,opt=rep(0,params['n']))
Tetra_p2r7r8_raw <- res[grep("w",names(res))]

##### save data for compiled data plot (havnet calculated mean of this replica yet)
save(Tetra_p2r7r8_raw, file="Tetra_p2r7r8_raw.RData")
...
# 
# calc_data <- sapply(Tetra_p2r7r8_raw,FUN=function(x) {c(m=mean(x),sd=sd(x),se=sd(x)/sqrt(nsamps),me=t_score*sd(x)/sqrt(nsamps),up=mean(x)+t_score*sd(x)/sqrt(nsamps),lb=mean(x)-t_score*sd(x)/sqrt(nsamps))})
# 
...
# 
# complete_data <- cbind(phenotype=c("trait"),ploidy=c("4X"),line=c("p6r5r5"),type=c("P1","P2","F1","F2","F3","F4","F5","F6","F7","B1","B2"),as.data.frame(t(calc_data)),n=c("100"))
# library("writexl")
# write_xlsx(complete_data, "~/Desktop/Cross_output/F1-7_output/Tetra_F1-7/Tetra_p2r1r2.xlsx")

# 
# 
# 
# save(plot_data, file="Tetra_p6r5r1.RData")
# 
